...
map:
    ...
    lw t1, 0(s0)        # load the address of the array of current node into t1 [mistake1]
    lw t2, 4(s0)        # load the size of the node's array into t2
mapLoop:         
    lw a0, 0(t1)        # load the value at that address into a0
    
    addi sp, sp, -12    # [mistake3]
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp)
    ...
    addi t1, t1, 4      # offset the array address by the count [mistake2]
    bne t0, t2, mapLoop # repeat if we haven't reached the array size yet
    
    lw a0, 8(s0)        # load the address of the next node into a0 [mistake4]
    mv a1, s1        # put the address of the function back into a1 to prepare for the recursion [mistake5]
